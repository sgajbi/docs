...
## Example 86. Cash account closure residual balance

### Scenario

A client requests cash account closure after all expected payments appear complete. A small residual balance remains because interest accrued after the closure quote date and a correspondent fee posted late.

| Attribute | Value |
|---|---:|
| Closure quote balance | 0 |
| Late credit interest | 42.75 |
| Late correspondent fee | 15.00 |
| Residual balance | 27.75 |
| Closure status | Pending residual sweep |

### Residual balance

```text
residual_balance = late_credit_interest - late_correspondent_fee
residual_balance = 42.75 - 15.00 = 27.75
closure_allowed = residual_balance == 0 or residual_disposition_instruction_confirmed
```
...
| Area | Treatment |
|---|---|
| Closure state | Preserve closure request, quote date, final ledger balance, residual events and disposition instruction. |
| Cash availability | Do not suppress residual cash merely because the account is pending closure. |
| Operations | Route residual balance to sweep, fee write-off, donation, transfer or client instruction workflow by policy. |
| Reporting | Show residual cash as closure residual, not as ordinary investable cash. |
| Audit | Keep late postings and final closure approval linked to the account closure case. |

### QA assertions

| Test | Expected result |
|---|---|
| Late interest posts after closure quote | Residual balance reopens or keeps closure case pending. |
| Residual disposition is missing | Final closure is blocked. |
| Residual is transferred | Cash movement links to closure case and client instruction. |
| Client report is generated | Residual cash is labelled separately from active cash. |

## Example 87. FX sweep rounding residual

### Scenario

An automated FX sweep converts foreign-currency cash into the reporting currency. Decimal precision and minimum trade-size rules leave a small unconverted residual.

| Attribute | Value |
|---|---:|
| EUR cash before sweep | 10,000.00 |
| EUR amount converted | 9,999.97 |
| EUR residual | 0.03 |
| EUR/USD execution rate | 1.0850 |
| Minimum follow-up trade size | 25.00 |

### Residual calculation

```text
fx_sweep_residual = source_currency_cash - converted_source_amount
fx_sweep_residual = 10,000.00 - 9,999.97 = 0.03
follow_up_trade_allowed = fx_sweep_residual >= minimum_follow_up_trade_size
```
...
| Area | Treatment |
|---|---|
| Sweep state | Preserve sweep rule, source cash, converted amount, rate, rounding precision and residual. |
| Cash view | Keep residual in source currency until it is swept, written off or accumulated by policy. |
| Performance | Attribute realized FX conversion and residual translation separately. |
| Advisory | Avoid showing tiny residuals as actionable trade recommendations when below trade minimum. |
| Reporting | Explain residual as rounding/minimum-size outcome, not failed conversion. |

### QA assertions

| Test | Expected result |
|---|---|
| Residual is below minimum trade size | No forced follow-up FX trade is created. |
| Residuals accumulate above threshold | Sweep candidate is generated by policy. |
| Sweep rate is restated | Converted cash and residual translation remain traceable. |
| Client report is generated | Source-currency residual is visible and explained. |

## Example 88. Sanctions release after cut-off

### Scenario

A payment is held by screening and released after the market cut-off. Funding was available, but the release time means same-day settlement is no longer possible.
...
| Currency | USD |
| Standard cut-off | 16:00 |
| Screening release time | 16:35 |
| Funding status | Available |
| Settlement outcome | Next business day |

### Release rule

```text
same_day_release_allowed = screening_clear_time <= market_cutoff and funding_available
next_day_roll_required = screening_clear_time > market_cutoff
```
...
| Area | Treatment |
|---|---|
| Payment state | Preserve screening hold, release timestamp, cut-off, funding status and revised value date. |
| Cash reservation | Keep reserved cash until payment is cancelled, released or value-date rolled. |
| Settlement | Roll settlement to next valid business day unless a documented cut-off extension exists. |
| Client communication | Explain delay as post-cut-off screening release, not funding failure. |
| Controls | Track repeated post-cut-off releases and extension approvals. |

### QA assertions

| Test | Expected result |
|---|---|
| Screening clears after cut-off | Same-day settlement is blocked or rolled. |
| Cut-off extension is approved | Release uses scoped extension evidence. |
| Funding is unavailable | Payment remains blocked even if screening clears. |
| Report is generated | Held, released and rolled states are distinct. |

## Example 89. Deposit break quote expiry

### Scenario

A client requests early break of a term deposit. Treasury provides a break quote with a short validity window. The client accepts after the quote expires.

| Attribute | Value |
|---|---:|
| Deposit principal | 500,000 |
| Original maturity interest | 12,000 |
| Break penalty quote | 3,250 |
| Quote validity | 30 minutes |
| Acceptance time after quote | 42 minutes |

### Quote validity

```text
quote_expired = acceptance_time > quote_expiry_time
estimated_break_proceeds = principal + accrued_interest_to_break - break_penalty_quote
release_allowed = not quote_expired or renewed_quote_confirmed
```
...
| Area | Treatment |
|---|---|
| Quote state | Preserve quote timestamp, expiry, penalty, accrued interest, approver and client acceptance time. |
| Execution | Do not execute expired break quote without renewed treasury confirmation. |
| Suitability | Re-check liquidity need, penalty impact and alternative funding before re-quote. |
| Reporting | Show quoted, expired, renewed and executed states distinctly. |
| Audit | Keep original and renewed quote evidence with the final deposit break. |

### QA assertions

| Test | Expected result |
|---|---|
| Acceptance arrives after expiry | Break execution is blocked or re-quoted. |
| Renewed quote differs | Client-facing proceeds update with new evidence. |
| Client cancels after re-quote | Deposit remains active with no break cash posted. |
| Report is generated | Expired quote is not shown as confirmed proceeds. |

## Example 90. Money-market fund redemption settlement delay

### Scenario

A money-market fund redemption is expected to settle T+0, but the transfer agent confirms a settlement delay. The cash projection must move from expected available cash to pending receivable.

| Attribute | Value |
|---|---:|
| Redemption amount | 750,000 |
| Expected settlement date | T+0 |
| Revised settlement date | T+1 |
| Linked payment due | 700,000 |
| Confirmed cash before redemption | 120,000 |

### Available cash impact

```text
available_cash_for_payment = confirmed_cash_before_redemption
projected_cash_after_redemption = confirmed_cash_before_redemption + redemption_amount
payment_shortfall_without_redemption = linked_payment_due - confirmed_cash_before_redemption
payment_shortfall_without_redemption = 700,000 - 120,000 = 580,000
```
...
| Area | Treatment |
|---|---|
| Redemption state | Preserve order, expected settlement, transfer-agent confirmation and revised settlement date. |
| Cash projection | Move delayed settlement from available cash to pending receivable. |
| Payment control | Do not fund same-day payment from delayed redemption without approved bridge funding. |
| Liquidity | Escalate linked payment shortfall or resize payment by confirmed cash. |
| Reporting | Explain redemption as pending settlement, not failed order. |

### QA assertions

| Test | Expected result |
|---|---|
| Transfer agent confirms delay | Redemption cash moves out of available cash. |
| Linked payment depends on redemption | Shortfall or bridge-funding workflow opens. |
| Settlement arrives T+1 | Pending receivable converts to ledger cash. |
| Client report is generated | Pending fund redemption and available cash are distinct. |

## Example 91. Treasury counterparty settlement-limit exhaustion

### Scenario

Treasury attempts to place surplus cash with a counterparty, but the counterparty settlement limit is nearly consumed by other same-day flows.

| Attribute | Value |
|---|---:|
| Counterparty settlement limit | 5,000,000 |
| Used limit before placement | 4,650,000 |
| Proposed placement | 600,000 |
| Remaining limit | 350,000 |
| Excess over limit | 250,000 |

### Limit check

```text
remaining_counterparty_limit = settlement_limit - used_limit_before_placement
remaining_counterparty_limit = 5,000,000 - 4,650,000 = 350,000
limit_excess = proposed_placement - remaining_counterparty_limit
limit_excess = 600,000 - 350,000 = 250,000
release_allowed = limit_excess <= 0 or approved_limit_exception
```
...
| Area | Treatment |
|---|---|
| Limit state | Preserve counterparty limit, used amount, pending flows, proposed placement and exception state. |
| Execution | Block, resize or route placement when counterparty limit is exhausted. |
| Liquidity | Consider alternate approved counterparties or split placement by policy. |
| Risk | Keep concentration, credit and settlement-limit checks separate from client cash availability. |
| Reporting | Explain delayed or resized placement as counterparty-limit control outcome. |

### QA assertions

| Test | Expected result |
|---|---|
| Proposed placement exceeds remaining limit | Placement is blocked, resized or exceptioned. |
