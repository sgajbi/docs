# 09 - Source Notes and Further Reading

This pack is written as a professional study and platform-design reference. It combines general financial knowledge, wealth-platform design practice, and investor/regulatory education sources.

## 1. Investor and regulatory education references

### Margin accounts and margin risk

- FINRA - Margin Accounts / Margin Regulation: explains buying securities using borrowed funds from a securities firm and the need for a margin account.
- Investor.gov / SEC - Understanding Margin Accounts: explains that margin accounts can be risky and may not be appropriate for everyone; losses can be larger than the amount invested.
- Investor.gov / SEC - Margin Account glossary: defines margin account as a brokerage account where the broker-dealer lends cash using the account as collateral.
- Interactive Brokers - Margin Education / Risk Disclosure: useful practitioner reference for maintenance margin, excess liquidity and liquidation concepts.

## 2. Collateral, haircuts and credit-risk mitigation

- BIS Basel Framework - Credit Risk Mitigation: describes recognition of collateral and adjusted exposure concepts in regulatory capital frameworks.
- OCC - Margin Methodology: practitioner reference for margin methodology and collateral haircuts in clearing context.
- MAS / Singapore regulatory resources - margin financing and securities lending rules can affect local implementation.

## 3. Wealth lending and portfolio lending references

- Standard Chartered Private Bank - wealth lending / securities-based lending education materials.
- HSBC Singapore - Wealth Portfolio Lending product sheet: example of revolving, uncommitted loan secured by eligible assets.
- DBS Singapore - margin trading guide: useful local explanation of leverage, margin maintenance and buffer concepts.

## 4. Internal knowledge-base cross-references

This pack should be studied together with:

- Notes Reference Pack
- Bonds Reference Pack
- Funds Reference Pack
- Equities Reference Pack
- Derivatives Reference Pack
- Structured Products Reference Pack
- Cash, Deposits, Money Market and FX Reference Pack
- Private Markets and Alternatives Reference Pack
- Cross-Product Canonical Position and Transaction Model Addendum

## 5. Key source URLs

- FINRA Margin Accounts: https://www.finra.org/rules-guidance/key-topics/margin-accounts
- SEC Investor.gov Understanding Margin Accounts: https://www.investor.gov/introduction-investing/general-resources/news-alerts/alerts-bulletins/investor-bulletins-29
- SEC Investor.gov Margin Account glossary: https://www.investor.gov/introduction-investing/investing-basics/glossary/margin-account
- BIS Credit Risk Mitigation framework: https://www.bis.org/basel_framework/chapter/CRE/22.htm
- OCC Margin Methodology: https://www.theocc.com/risk-management/margin-methodology
- DBS Singapore Quick Guide to Margin Trading: https://www.dbs.com.sg/personal/articles/nav/investing/quick-guide-to-margin-trading
- HSBC Singapore Wealth Portfolio Lending product sheet: https://www.hsbc.com.sg/content/dam/hsbc/sg/documents/loans/wealth-portfolio-lending-purpose-loan.pdf
- Standard Chartered Understanding Wealth Lending: https://www.sc.com/en/uploads/sites/66/content/docs/wm-thematic-report-understanding-wealth-lending-strategies-for-growing-your-financial-portfolio-privatebank-06-november-2025.pdf

## 6. Quality standard for future expansion

Future versions should add:

- jurisdiction-specific lending rules where needed
- legal pledge agreement examples at conceptual level
- detailed liquidation workflow variants
- Lombard lending examples by asset class
- derivative collateral netting examples
- advanced optimization for shared collateral allocation
- product eligibility matrix by mandate and risk profile
- golden test data files for buying-power engines
- API contracts and event schemas
- sequence diagrams for pre-trade checks and margin-call workflows
