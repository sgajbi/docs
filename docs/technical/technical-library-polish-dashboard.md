...
| Observability, SRE and supportability | Strong | Structured logs, metrics, traces, health/readiness, dashboards, SLOs, alerting, runbooks, incidents, diagnostics, sensitive-data boundaries and incident supportability case studies. | Add examples for alert route tuning, dashboard acceptance checks and no-sensitive-telemetry gates. |
...
| 2 | Alert design, dashboard acceptance and no-sensitive-telemetry case studies. | These extend the incident supportability case study into more specialized operational controls. |
